 ...
 *
 */
public class FirmaServidorCounterSign extends StartingClass implements WebServicesAvailable
{
	...
	private static final String errorMessage = 
		...
		"> FirmaServidorCounterSign idAplicacion idTransaccion aliasCertificadoServidor [-r referenciaExterna] [-f firmanteObjetivo] [-a algoritmoHash]\n\n" +
		"donde\n" +
	...
	public static void main (String[] args)
	{
		...
		new FirmaServidorCounterSign().run(args);
	}
	...
	public void run(String[] args)
	{
		...
		System.out.println(".[Preparando la petición al servicio Web " + serverSignatureCounterSignWebServiceName + "...]");
		Document serverSignatureCounterSignRequest = null;
		try {
			if(objetiveSigner != null)
				serverSignatureCounterSignRequest = UtilsWebService.prepareServerSignatureCounterSignRequest(appId, idTransaction, aliasServerCert, referenceId, hashAlgorithm, base64Coder.encodeBase64(objetiveSigner));
			else
				serverSignatureCounterSignRequest = UtilsWebService.prepareServerSignatureCounterSignRequest(appId, idTransaction, aliasServerCert, referenceId, hashAlgorithm, null);
		} catch (Exception e1) {
			System.err.println("Error codificando el firmante objetivo");
			System.exit(-1);
		}
		...
		System.out.println("..[/peticion]");
		String response = UtilsWebService.launchRequest(serverSignatureCounterSignWebServiceName, serverSignatureCounterSignRequest);
		...
		signatureFormat = UtilsWebService.getInfoFromDocumentNode(response, "formatoFirma");
