...
public class BlockUserSignature extends StartingClass implements WebServicesAvailable
{
	...
	private static final String errorMessage = 
		...
		"> BlockUserSignature idAplicacion aliasCertificadoServidor [-f ficherosAFirmar] [-t transaccionesFirma] [-m transaccionBloque_idTransaccionesDocumentos] [-r referenciaExterna] [-a algoritmoHash] [-fe formatoFirmaElectronica] [-ef etenderFormatoFirma]\n" +
		"\n" +
		"donde\n" +
...
//		"                                    Valores posibles: ES-T" +
	...
	{			 
		...
		new BlockUserSignature().run(args);
	}
	...
	public void run(String[] args)
	{
		...
		if (fileToBeSigned!=null)
		{
			...
			for (int i=0; i<fileToBeSigned.length; i++)
			{
				...
				docIds[i] = UtilsWebService.getInfoFromDocumentNode(response, "documentId");
		...
		System.out.println(".[Preparando la petición al servicio Web " + blockUserSignatureF1WebServiceNameEng + "...]");
		Document blockUserSignatureF1Request = UtilsWebService.prepareBlockUserSignatureF1RequestEng(appId, docIds, idTransactions, aliasServerCert, selectiveBlocks, hashAlgorithm);
		System.out.println(".[/Petición correctamente preparada]");		
		...
		System.out.println("..[/peticion]");
		response = UtilsWebService.launchRequest(blockUserSignatureF1WebServiceNameEng,blockUserSignatureF1Request);
		System.out.println("..[respuesta]");
		System.out.println(response);
		System.out.println("..[/respuesta]");
		...
		//BeanSalidaBrowser appletOut =  BrowserAfirma.navegar(hashBlockSignature,0,signatureFormatForClient,hashAlgorithm+"WITHRSAENCRYPTION");
		...
		System.out.println(".[Preparando la petición al servicio Web " + blockUserSignatureF3WebServiceNameEng + "...]");
		Document blockUserSignatureF3Request = UtilsWebService.prepareBlockUserSignatureF3RequestEng(appId, transactionId, signatureValue, signCertificateValue,signatureFormat, updateSignatureFormat, referenceId);
		System.out.println(".[/Petición correctamente preparada]");
		...
		System.out.println("..[/peticion]");
		response = UtilsWebService.launchRequest(blockUserSignatureF3WebServiceNameEng,blockUserSignatureF3Request);
		System.out.println("..[respuesta]");
		System.out.println(response);
		System.out.println("..[/respuesta]");
		if (!UtilsWebService.isCorrectEng(response)){
