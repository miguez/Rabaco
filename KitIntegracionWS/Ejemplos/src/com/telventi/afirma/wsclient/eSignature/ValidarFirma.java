...
public class ValidarFirma extends StartingClass implements WebServicesAvailable
{
	...
	private static final String errorMessage = 
		...
		"ValidarFirma idAplicacion firmaElectrónicaAValidar formatoFirmaElectronica modo [-fh fichero/hash] [-ah algoritmoHash]\n" +
		"\n" +
		"donde\n" +
	...
	public void run(String[] args)
	{
		...
		{
			if (mode == 0)
				eSignatureValidationRequest = UtilsWebService.prepareValidateSignatureRequest(appId, new String(eSignature), eSignatureFormat, null, null, null);
			else if (mode == 1)
				eSignatureValidationRequest = UtilsWebService.prepareValidateSignatureRequest(appId, new String(eSignature), eSignatureFormat, null, null, data);
			else if (mode == 2)
				eSignatureValidationRequest = UtilsWebService.prepareValidateSignatureRequest(appId, new String(eSignature), eSignatureFormat, hash, hashAlgorithm, null);
			else
				eSignatureValidationRequest = UtilsWebService.prepareValidateSignatureRequest(appId, new String(eSignature), eSignatureFormat, hash, hashAlgorithm, data);
		}
		catch (Exception e)
		{
		...
		System.out.println("..[/peticion]");
		String response = UtilsWebService.launchRequest(signatureValidationWebServiceName, eSignatureValidationRequest);
		...
		String descriptionValue = UtilsWebService.getXMLChildsFromDocumentNode(response, "descripcion");
