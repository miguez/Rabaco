...
public class FirmaServidor extends StartingClass implements WebServicesAvailable
{
	...
	private static final String errorMessage = 
		...
		"> FirmaServidor idAplicacion ficheroAFirmar aliasCertificadoServidor [-r referenciaExterna] [-f formatoFirmaElectronica] [-a algoritmoHash] [-i incluirDocumento]\n" +
		"\n" +
		"  donde\n" +
	...
	private void processIncludeDocument(){
		...
		System.out.println(".[Preparando la petición al servicio Web " + serverSignatureWebServiceName + "...]");
		Document serverSignatureRequest = UtilsWebService.prepareServerSignatureIncDocRequest(appId, new String(fileContentToBeSigned), aliasServerCert, referenceId, hashAlgorithm, signatureFormat,fileName,fileType);
	...
	private void processNoIncludeDocument(){
		...
		Document custodyDocumentRequest = UtilsWebService.prepareCustodyDocumentRequest(appId, fileName, fileType, new String(fileContentToBeSigned));
		System.out.println(".[/Petición correctamente preparada]");
		...
		// Preparación de la petición al servicio Web de FirmaServidor
		System.out.println(".[Preparando la petición al servicio Web " + serverSignatureWebServiceName + "...]");
		Document serverSignatureRequest = UtilsWebService.prepareServerSignatureRequest(appId, docId, aliasServerCert, referenceId, hashAlgorithm, signatureFormat);
		System.out.println(".[/Petición correctamente preparada]");
	...
	private void launchServerSignatureRequest(Document serverSignatureRequest, String docId,String fileType){
		...
		System.out.println("..[/peticion]");
		String response = UtilsWebService.launchRequest(serverSignatureWebServiceName, serverSignatureRequest);
		System.out.println("..[respuesta]");
		System.out.println(response);
		System.out.println("..[/respuesta]");		
		if (!UtilsWebService.isCorrect(response))
