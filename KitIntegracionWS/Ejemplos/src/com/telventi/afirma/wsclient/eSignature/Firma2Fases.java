...
public class Firma2Fases extends StartingClass implements WebServicesAvailable {
	...
	private static final String errorMessage =
		...
		"> Firma2Fases idAplicacion [-r referenciaExterna] [-fe formatoFirmaElectronica] [-a algoritmoHash] [-cd custodiarDocumento]\n\n" +
		"  donde\n" +
	...
	public void run(String[] args) {
		...
		Document twoPhasesUserSignatureF2Request = UtilsWebService.prepareTwoPhasesUserSignatureF2Request(appId, signatureValue, signCertificateValue, signatureFormat, fileContentToBeSigned, fileType, fileName, hashAlgorithm, updateSignatureFormat, referenceId,custodyDoc);
		...
		System.out.println(XMLUtils
				.DocumentToString(twoPhasesUserSignatureF2Request));
		System.out.println("..[/peticion]");
		String response = UtilsWebService.launchRequest(twoPhasesUserSignatureF2WebServiceName, twoPhasesUserSignatureF2Request);
		...
		try {
			...
			while (i < args.length) {
				...
				if (args[i].equals("-r")) {
					...
					i++;
					custodyDoc = Boolean.getBoolean(args[i]);
					i++;
